...
@RestController
@RequestMapping(BaseKwsFishsixAPI.PREFIX_PATH + "/study-class/evaluation/reject")
public class StudyClassEvaluationRejectAPI extends BaseKwsFishsixAPI
{
	@Autowired
	...
	@PostMapping
	public ResponseEntity<?> rejectStudyClassEvaluation(HttpServletRequest request, @RequestBody RequestMessage requestMessage)
	{
		logger.info("### {}.rejectStudyClassEvaluation ###", getClass().getSimpleName());
		
		StudyClassEvaluationRejectCommand.Response approveEvaluationResponse = studyClassService.rejectStudyClassEvaluation(req -> {
